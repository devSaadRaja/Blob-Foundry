...
contract Feeding is Ownable {
    ...
    function setBlobSlippage(uint256 newSlippage) external onlyOwner {
        require(
            newSlippage >= 1e3 && newSlippage <= 100e4,
    ...
    function claim(uint256 _index) external vestingExists(msg.sender, _index) {
    ...
    function _calculateReward(
        ...
        require(
            tokenPath[WETH].length >= 2 &&
                tokenPath[WETH][0] == WETH &&
                tokenPath[WETH][1] == BLOB,
